...
const BrandPage = () => {
  ...
  const onFinish = async (item) => {
    ...
    if (item.image && item.image.length > 0 && item.image[0].originFileObj) {
      formData.append("image", item.image[0].originFileObj);
    } else {
      if (item.image_remove) {
  ...
  const handleEdit = (data) => {
    ...
    // Define the base URL dynamically to ensure it works in any environment.
    // This gets the protocol, domain, and port of your current page.
    const baseUrl = window.location.origin;

    // The image URL on the server
    const imageUrl = data.image ? `${baseUrl}/storage/${data.image}` : null; 
    const files = imageUrl
      ? [
          {
            uid: data.id, // A unique ID for the file
            name: data.image, // The filename
            status: "done",
            url: imageUrl, // The full, accessible URL of the image
          },
        ]
      : [];
